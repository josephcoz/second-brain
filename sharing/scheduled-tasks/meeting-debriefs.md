...
For each meeting, you'll often hit attendees with no `People/` file or topics with no `Topics/` node. **Do not fabricate context. Do not silently drop them.** Surface them in a per-meeting `#### Open Questions` subsection, so the next interactive session can engage the Learning Loop (see `~/github-projects/second-brain/CLAUDE.md`) and persist answers to the right vault location.

You cannot ask the user mid-run — surface, don't block.

Examples:

- **Unknown attendee:** "I have no `People/` file for Jackson Lieu (AE candidate). What's the role he's interviewing for, and who else should weigh in?"
- **Undocumented topic:** "The meeting title says 'Pivot Partner Enablement' but I can't find any vault coverage. Is this part of `[[Pricing and Packaging]]` or its own workstream?"
- **Stale relationship context:** "`People/Sarah Chan.md` was last updated 6 weeks ago and her role context may be outdated. Worth a refresh?"

Skip this section for any meeting where you have full vault coverage. Don't pad the output.
...
#### Open Questions
- [Unfamiliar attendees, undocumented meeting topics, or stale context. Omit this subsection if vault coverage is complete. See "Context Gap Surfacing" above.]
