...
2. Enrich meeting files with Google Meet transcripts/notes from Google Drive (Step 1 — via local `gdrive-transcripts.py` script, primary content source)
...
Google Meet auto-saves two types of docs to Drive:
- **Gemini Notes** (`{Title} - {Date} - Notes by Gemini`) — AI-generated summaries with structured notes. Preferred when available.
- **Transcripts** (`{Title} - {Date} - Transcript`) — raw speaker-attributed transcripts.

These are the **primary** content source for meeting files — Granola notes are the **backup** (only used when no Drive doc exists).

### 1a. Find today's meetings that need content

Identify meeting files from today that don't already have a Drive transcript:
...
### 1b. Fetch Google Drive docs using the local script

Run the sync command to search for and fetch all Google Meet docs from today:

```bash
source {{work_automation_repo}}/.venv/bin/activate
python3 {{work_automation_repo}}/scripts/gdrive-transcripts.py sync --date $(date +%Y-%m-%d)
```

This outputs a JSON array of objects, each with:
- `id` — Google Doc ID
- `meetingTitle` — parsed meeting title
- `meetingDate` — YYYY-MM-DD
- `docType` — `"gemini_notes"` (preferred) or `"transcript"`
- `content` — full document text

If a meeting has both Gemini notes and a raw transcript, the script de-duplicates automatically — only the Gemini notes are returned.

### 1c. Match Drive docs to meeting files

For each Drive doc, match it to a meeting file from Step 1a using this logic:

1. **Match by date + title overlap**: Compare the Drive doc's `meetingTitle` against today's meeting filenames. Normalize both to lowercase, split into words, and check if at least 50% of the shorter title's words appear in the longer one.

2. **Greedy assignment**: Sort matches by overlap score descending. Each Drive doc and each meeting file can only be matched once.

### 1d. Write Drive content to meeting files

For each matched pair:

1. Update the meeting file:
   - Add `transcript_source: google_meet` to the YAML frontmatter
   - Add `transcript_doc_id: {id}` to the YAML frontmatter
   - Add `transcript_type: {docType}` to the YAML frontmatter
   - If `docType` is `gemini_notes`: replace `## Notes` section with `## Google Meet Notes` containing the content
   - If `docType` is `transcript`: replace `## Notes` section with `## Transcript` containing the content
   - If the file had Granola notes content (not the placeholder), preserve it as a `## Granola Notes` section below the Drive content

2. Log the result: `log(f"ENRICHED: {filename} with Drive {docType} {id}")`

### 1e. Fallback hierarchy

For any meeting files from today that did NOT get a Drive doc match:
1. **Granola notes**: If Granola export wrote real content (not the "*AI summary not yet available*" placeholder), leave it as-is.
2. **Empty**: If no content from any source, leave the placeholder. It may get enriched on the next run.
...
## Granola Notes
[Preserved Granola AI summary, if it existed]
```
...
While writing the journal, you'll encounter people, projects, terms, or systems that have no vault coverage. **Do not fabricate context. Do not silently drop them.** Surface them in a `## Open Questions for {{user}}` section near the bottom of the journal entry (just before `## Related Topics`), so the next interactive session can engage the Learning Loop (see `~/github-projects/second-brain/CLAUDE.md`) and persist the answers to the right vault location.
...
- **Surface gaps, don't fabricate.** When you encounter unfamiliar people, topics, or systems, add them to `## Open Questions for {{user}}` (see Context Gap Surfacing) — never invent context to fill the gap
